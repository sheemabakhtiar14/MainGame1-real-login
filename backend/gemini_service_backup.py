...
# Expanded fallback email questions from the frontend with more variety
...
Your bank account has been temporarily suspended due to suspicious activity. To restore access, please verify your account information immediately.
...
Bank of America Security Team""",
        "level": 1,
        "isScam": True,
        "explanation": "This is a scam email. The domain uses a zero (0) instead of the letter 'o' in 'bank0famerica.com'. Bank of America's official domain is 'bankofamerica.com'. The urgent language and short timeframe are also red flags."
    },
...
1-888-BEST-BUY""",
    ...
    {
        "id": "email-2-3",
        "subject": "Google Drive: Document shared with you",
        "sender": "drive-shares-noreply@google.com",
        "content": """Someone shared a document with you on Google Drive.

Document: "Financial_Report_Q3_2023.pdf"
Shared by: john.manager@yourcompany.com

[View Document]

This document will expire in 7 days. Please view it before then.

The Google Drive Team""",
        "level": 2,
        "isScam": False,
        "explanation": "This appears to be a legitimate Google Drive sharing notification. It comes from an official Google domain, has reasonable content, and doesn't use urgent or threatening language. However, always verify the sender and be cautious with shared documents."
    },
...
Microsoft Billing Department""",
    ...
    {
        "id": "email-3-2",
        "subject": "Weekly Security Digest from GitHub",
        "sender": "noreply@github.com",
        "content": """GitHub Security Digest - Week of June 5, 2023

Hello Developer,

Here's your weekly security summary for your repositories:

• 3 dependency updates available
• 1 security advisory affects your repositories
• No suspicious access detected

[View Full Security Report]

To unsubscribe from these notifications, update your email preferences in your GitHub settings.

GitHub Security Team""",
        "level": 3,
        "isScam": False,
        "explanation": "This appears to be a legitimate security digest from GitHub. It uses the official GitHub domain, has appropriate technical content for developers, and provides a clear unsubscribe mechanism. The tone is professional and informative."
    }
]
...
class GeminiService:
    def __init__(self, model_name="gemini-1.5-pro"):
        ...
        self.use_fallback_first = True  # Use realistic examples before API
        self.api_call_count = 0
        self.max_api_calls_per_hour = 50  # Conservative limit
        self.cache = {}  # Simple in-memory cache
        self.fallback_ratio = 0.8  # 80% fallback, 20% API calls
        ...
        try:
            ...
            print(f"📊 Quota optimization enabled - {self.fallback_ratio*100}% fallback ratio")
    ...
    def _load_training_data(self, filename: str) -> str:
      ...
      def _call_gemini(self, prompt: str, context: str = "") -> str:
        """Make optimized API call to Gemini with quota management"""
        # Check if we should use fallback first
        if self.use_fallback_first and random.random() < self.fallback_ratio:
            return "FALLBACK_REQUESTED"
        
        # Check API call limit
        if self.api_call_count >= self.max_api_calls_per_hour:
            print(f"⚠️ API call limit reached ({self.max_api_calls_per_hour}/hour), using fallback")
            return "FALLBACK_REQUESTED"
      ...
      def generate_email_scam(self, level: int) -> Dict[str, Any]:
        ...
        # Use fallback content most of the time to save quota
        return self._generate_fallback_content('email', level)
        
        # This code below is kept for when we want to occasionally use API
        difficulty_context = {
        ...
        prompt = f"""Generate email scam for level {level}. Return JSON with: subject, sender, content, isPhishing:true, redFlags, explanation."""
        
        response = self._call_gemini(prompt, context)
        if response == "FALLBACK_REQUESTED":
            return self._generate_fallback_content('email', level)
      ...
      def generate_url_scam(self, level: int) -> Dict[str, Any]:
        ...
        prompt = f"""Generate URL scam for level {level}. Return JSON with: url, description, legitimateUrl, isPhishing:true, redFlags, explanation."""
    ...
    def _get_fallback_feedback(self, is_correct: bool, scam_type: str) -> str:
      ...
      def generate_legitimate_email(self, level: int) -> Dict[str, Any]:
        ...
        # Use fallback content most of the time to save quota
        return self._generate_fallback_content('email_legitimate', level)
        ...
        prompt = f"""Generate legitimate email for level {level}. Return JSON with: subject, sender, content, isPhishing:false, trustIndicators, explanation."""
        ...
        return result    def generate_legitimate_url(self, level: int) -> Dict[str, Any]:
        """Generate legitimate URL - optimized with fallback-first approach"""
        # Use fallback content most of the time to save quota
        return self._generate_fallback_content('url_legitimate', level)
        ...
        prompt = f"""Generate legitimate URL for level {level}. Return JSON with: url, description, legitimateUrl, isPhishing:false, trustIndicators, explanation."""
