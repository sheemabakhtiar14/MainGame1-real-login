...
const StandardMode: React.FC = () => {
  ...
  const [currentQuestions, setCurrentQuestions] = useState(standardQuestions.filter(q => q.level === state.level));
  const [currentQuestion, setCurrentQuestion] = useState(currentQuestions[state.currentQuestion]);
          ...
          {!state.gameCompleted ? (
            <Card className="mb-6">
              <div className="p-6 border-b border-dark-700">
                <h3 className="text-lg font-medium text-white mb-3">Message from Unknown Sender:</h3>
                <p className="p-4 bg-dark-700 rounded-lg text-gray-300">{currentQuestion?.question}</p>
              </div>
              
              <div className="p-6">
                <h3 className="text-lg font-medium text-white mb-3">How would you respond?</h3>
                        ...
                        <div className={`p-4 rounded-lg ${isCorrect ? 'bg-green-900/20 border border-green-700' : 'bg-red-900/20 border border-red-700'}`}>
                          ...
                          {!isCorrect && (
                            <div className="mt-3 p-3 bg-dark-800 rounded border border-dark-700">
                              <p className="text-sm font-medium text-primary-400 mb-1">The correct response would be:</p>
          ...
          <Card className="p-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="text-center p-2">
                ...
                <div className="text-lg font-medium text-secondary-400">{state.score}</div>
              </div>
            </div>
          </Card>
